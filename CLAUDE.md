...
## Phase 2 / MVP 範囲外項目の管理規約(MANDATORY)

**設計文書を編集中、AI が「Phase 2 で〜」「MVP では扱わない」「将来拡張」を提案・記述しようとした場合、必ず以下の手順を踏むこと**。詳細仕様は `aidlc-docs/inception/id-index.md §6` を参照。

### 必須手順

1. **既存 P2-NN Story の検索** — `aidlc-docs/inception/user-stories/stories.md` の Phase 2 Epic セクションに該当テーマがあるか
2. **吸収または新規追加** — 既存があれば Story 概要に bullet 追記、なければ末尾に新規 `P2-NN` 行を追加
3. **id-index.md §1 を同期** — `P2-NN` 範囲を最新値に更新
4. **設計文書での記述** — `[Phase 2: P2-NN]` の形式で Story ID リンクのみ。**インラインに詳細仕様を書かない**

### 禁止パターン

- ❌ 設計文書に「Phase 2 で〜」とだけ書いて Story を作らない
- ❌ Phase 2 機能の詳細を data-model.md / application-design.md などに段落書きする(stories.md が一次情報源)
- ❌ 同じ Phase 2 アイデアを複数の設計文書に重複記述する

### 編集前チェックリスト

設計文書の編集前に「Phase 2 で〜」を含む記述を提案したくなった場合:

- [ ] 該当する `P2-NN` が stories.md にあるか確認したか
- [ ] なければ stories.md に新規 Story を追加したか
- [ ] あれば既存 Story の概要に項目を追記したか
- [ ] 設計文書のインライン記述を `[Phase 2: P2-NN]` 形式のリンクのみにしたか
- [ ] id-index.md §1 の P2-NN 範囲が最新値か確認したか

**この規約に違反した編集は即座に修正する**(レビュー指摘を待たない)。
