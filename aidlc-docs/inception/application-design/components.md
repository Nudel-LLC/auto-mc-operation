...
DDD レイヤ別のコンポーネント定義。Q1=B(中粒度・柔軟調整) に従い、**全 54 コンポーネント** を 5 レイヤに配置(Repository トレイトファミリー D-14 を 1 行に集約しているため、実質抽象は約 32)。
...
| レイヤ | 主要コンポーネント数 |
|--------|---------------------|
| Domain(Entity / VO / Service / Port / Error) | 21(D-1〜D-19 + D-18.5 OAuthExchanger + D-6.5 Office) |
...
> Repository トレイトファミリー(D-14)を 1 行に集約しているため、**実質的なドメイン抽象は約 32**(D-1〜D-19 + D-18.5 + D-6.5 のうち D-14 をサブトレイト分割した場合の総数)。Q1=B「ユニットに合わせて柔軟」「過細化避ける」方針に沿い、関連性の高いリポジトリは同じトレイトファミリーにまとめてある。
