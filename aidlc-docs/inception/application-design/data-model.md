...
| `user_agent_hash` | TEXT | NULL 可 | User-Agent の SHA-256 ハッシュ。本テーブルは append-only で 1 ユーザーあたり数件しか挿入されないため「短時間連続同意での乗っ取り検出」には使わない。実用は **(a) 集約レベルの Bot 大量登録検出**(異なる多数 user_id が同一 UA から同意 → 監査ダッシュボード F-14 で検出)、**(b) 否認防止の補助情報**、**(c) 再同意時の環境差分検出**(限定的)。生 UA はブラウザフィンガープリントになりうるためハッシュ化(同一性検証は可、復元不可) |
