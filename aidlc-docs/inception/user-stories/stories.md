...
- **AC-6 (P2 配慮: ガイド付き誘導)**:
  ...
  - **When** Bot がセットアップ用 URL を返すとき
  - **Then** 1) ボタン形式で「Google と連携を始める」と表記(「OAuth 認可」と書かない)、2) 想定所要時間(例: 約 1 分)を併記、3) 完了後に LINE に戻る方法も明記する

**Traceability**:
- Implements: FR-1, FR-3, FR-4, FR-6, FR-7, NFR-4 SECURITY-01 / SECURITY-06 / SECURITY-08, NFR-5, F-08(P2 配慮)
...
**Acceptance Criteria**:
- **AC-1 (用語の言い換え辞書)**:
  - **Given** ユーザー向け文言テンプレート(i18n / メッセージ定数化)
  - **When** 開発者が文言を書く
  - **Then** 「OAuth 認可」「API キー」「LLM」「トークン」等の専門用語が **使われていないか lint** で検出できる(代替: 「Google と連携」「AI で自動分析」等)
- **AC-2 (失敗時の次アクション必須化)**:
  - **Given** エラー通知メッセージ
  - **When** メッセージを生成する
  - **Then** 「次に何をすればよいか」を **具体的なボタンまたは文言** として必ず含める(例: 「LINE で『再連携』とタップしてください」)
- **AC-3 (LINE 内完結の優先)**:
  - **Given** ユーザー操作フロー
  - **When** 設計する
  - **Then** 可能な限り **LINE トーク内で完結** させ、外部 URL に飛ばす場合は理由と所要時間を事前告知する
- **AC-4 (読みやすさ)**:
  - **Given** すべてのユーザー向け文言
  - **When** レビューする
...
|------|---------|----------------|--------------|----------------|
| Foundation | 8 | 2 (F-03, F-07) | 6 (F-01/02/04/05/06/08) | 0 |
...
| Phase 2 | 7 (概略) | — | — | — |
| **合計(MVP+Foundation)** | **55** | **36** | **6** | **13** |

> Plan の目安(60〜100)よりやや少ない 55 Story(F-08 = P2 配慮の Enabler 1 件追加)で MVP を網羅。粒度をさらに細かく要望される場合は次回ループで分割可能です。
