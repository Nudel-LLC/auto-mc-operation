...
- **テスト・監視のスコープ**: User Stories で扱うのは **テスト戦略の枠組み(F-13)と監視・アラート設計の枠組み(F-14)** までで、**個別テストケースの詳細・E2E シナリオ詳細・SLI/SLO の閾値調整・ダッシュボード設計** は **Application Design / Functional Design / Build and Test ステージ** で扱う(段階的な詳細化)
...
### Story F-13: テスト戦略・E2E テスト基盤の整備

**As a** サービス開発者
**I want** ユニットテスト / プロパティベーステスト(PBT)/ 結合テスト / E2E テスト の各層を **CI で自動実行できる基盤** を整える
**so that** 機能要件・非機能要件を機械的に守りつつ、リファクタや LLM プロンプト改良に伴う退行を早期検出できる
...
- **AC-1 (テストレイヤの定義)**:
  - **Given** プロジェクトのテスト戦略
  - **When** 設計
  - **Then** 以下 4 層を `docs/test-strategy.md` に明記し、各層の責務 / カバレッジ目標 / 実行頻度を定める:
    | レイヤ | 対象 | ツール | 実行タイミング |
    |--------|------|--------|---------------|
    | **ユニットテスト** | `domain` / `application` の純粋関数・ユースケース | `cargo test` | コミット毎(GitHub Actions) |
    | **プロパティベーステスト (PBT)** | 重複判定・スキーマ往復・CSV エンコード(NFR-6 の対象) | `proptest` | コミット毎 |
    | **結合テスト** | infrastructure アダプタ × ドメイン(Gmail/Calendar/LINE/Anthropic は WireMock で代替) | `cargo test --features integration` | PR 毎 |
    | **E2E テスト** | `local` 環境を起動 → モック Pub/Sub → モック LINE → 実際の DB / Queues を経由した end-to-end フロー | `cargo test --features e2e` + Playwright(Phase 2 で UI 追加時) | nightly + リリース前 |
- **AC-2 (テストカバレッジ目標)**:
  - **Given** ビジネスロジック層(`domain` / `application`)
  - **When** カバレッジ計測
  - **Then** `cargo llvm-cov` で 80% 以上(NFR-6 準拠)、未達は CI で fail
- **AC-3 (LLM プロンプト評価ハーネス)**:
  - **Given** U2-00 で整備した抽出プロンプトの評価ハーネス
  - **When** プロンプト変更時
  - **Then** ゴールデンセット(βテスター提供の実メール)に対する F1 スコアが閾値未満なら fail
- **AC-4 (E2E テストデータの管理)**:
  - **Given** E2E テストで使うフィクスチャ(模擬メール・模擬カレンダー予定)
  - **When** `tests/fixtures/e2e/` に配置
  - **Then** 個人情報を含まない合成データ(βテスター実データの匿名化版)で構成、リポジトリにコミット可能

**Traceability**:
- Implements: NFR-6(信頼性・テスト)

---

### Story F-14: 監視・アラート設計と運用基盤の整備

**As a** サービス開発者・運用者
**I want** サービス稼働状況を **メトリクス・ログ・アラート** で常時監視する基盤を整える
**so that** 障害を早期検出し、NFR-3(可用性 99.0% MVP / 99.5% 拡張時)を満たせる
...
- **AC-1 (メトリクスの定義)**:
  - **Given** 監視すべきサービス指標
  - **When** 設計
  - **Then** 以下を `docs/observability.md` に明記し、Cloudflare Workers Analytics Engine + Logpush で収集する:
    | 種別 | 指標例 |
    |------|--------|
    | **可用性** | Webhook 受信成功率(LINE/Pub/Sub)/ Worker invocation 成功率 |
    | **レイテンシ** | メール受信 → LINE 通知の P50/P95/P99(NFR-1: 5 分以内) |
    | **コスト** | Anthropic API 呼び出し回数 / 入出力トークン数 / 月次集計(NFR-7: 500円/ユーザー目標トラッキング) |
    | **業務指標** | 分類成功率 / 抽出 confidence 分布 / ルールベース判定率 |
    | **エラー率** | 失敗カテゴリ別カウント(F-06 logger / U2-EC-04 共通失敗ハンドラと連携) |
- **AC-2 (アラート設定)**:
  - **Given** メトリクス
  - **When** 異常を検知
  - **Then** 以下のアラートを **運用者の LINE グループ** に通知(F-04 staging Bot 経由):
    - DLQ にメッセージが滞留(>10 件 / 1 時間)
    - Webhook 失敗率 > 5%
    - レイテンシ P95 > 5 分(NFR-1 違反)
    - Anthropic API コスト月次予算超過警告(運用者のみ)
    - OAuth 失効率上昇(再認可キャンペーンの判断材料)
- **AC-3 (ログ集約)**:
  - **Given** F-06 で送出される構造化ログ・監査ログ
  - **When** 検索が必要
  - **Then** R2 に Logpush された JSON ログを **`wrangler tail` または外部 SIEM(将来検討)** で時系列クエリ可能にする
- **AC-4 (オペレーションランブック)**:
  - **Given** 想定される障害シナリオ
  - **When** 障害が発生
  - **Then** `docs/operations.md` に「アラート種別 → 確認手順 → 復旧コマンド」を記載(U1-EC-01 DLQ 復旧 / U2-EC-04 失敗カテゴリ別対応 / OAuth 失効再認可キャンペーン 等)

**Traceability**:
- Implements: NFR-3(可用性), NFR-7(コストトラッキング), NFR-8(監視・運用)
...
- **AC-2 (学習データ参照)**:
  - **Given** `entry_corpus` テーブルに過去エントリーメールが蓄積
...
- **AC-2 (本人帰属)**:
  - **Given** 収集メール
  - **When** 永続化
  - **Then** `entry_corpus` テーブルに保存(NFR-5: 本人作成文章は永続保管)
- **AC-3 (フィルタ)**:
...
**Acceptance Criteria**:
- **AC-1 (下書き生成 — 過去辞退メールから Few-shot 学習)**:
  - **Given** 重複する仮案件のリストと、過去にユーザーが送信した辞退メール(`entry_corpus` と並列の `decline_corpus` テーブル、Sent フォルダから自動収集)
  - **When** 辞退下書き生成が走る
  - **Then** **過去 3〜5 件の本人作成辞退メールを Few-shot 例としてプロンプトに含め**、ユーザーらしい言い回し・敬語レベル・文字数で各事務所宛の下書きを生成する
  - **Then** 過去事例が不足(3 件未満)の場合は、汎用テンプレート(「お世話になっております。〜大変恐縮ですが今回は辞退させていただきます。〜」)にフォールバック
- **AC-1.5 (事務所別の言い回し学習)**:
  - **Given** 同じ事務所宛の過去辞退メールがある場合
  - **When** プロンプト構築
  - **Then** その事務所宛の過去事例を優先的に Few-shot に採用(事務所ごとの呼称・文体に合わせる)
- **AC-2 (承認依頼)**:
  - **Given** 下書き生成完了
  - **When** LINE 通知
  - **Then** 「以下の辞退メールを送信しますか?[送信 / プレビュー / キャンセル]」が一覧で届く
- **AC-3 (辞退メール学習データの自動収集)**:
  - **Given** Gmail の Sent フォルダ
  - **When** 初期セットアップ時 + 定期的に
  - **Then** 「辞退」「失礼ながら」「お見送り」等のキーワードを含む過去送信メールを `decline_corpus` テーブルに保存(NFR-5: 本人作成文章は永続保管)

**Traceability**:
- Implements: FR-7, FR-5, NFR-5(本人作成データの保管)
...
| **P2-08** | 請求 CSV エクスポート(F8 = MVP 範囲外) | **MVP スコープ(Cl-2 D = A+B+C+D+F)に含めなかった F8 機能**。Phase 2 で実装。LINE Rich Menu / Quick Reply の **「請求CSV」ボタン → 月選択(今月 / 先月 / 月選択ピッカー)→ ダウンロード URL** のボタンのみフロー(F-08 メッセージング規約に準拠、テキスト入力ゼロ)。出力列・UTF-8 BOM・確定スロット 1 件出力等の詳細仕様は MVP 完成後に再定義 |
| **P2-09** | アカウントデータ管理拡張 | アカウント削除請求(A-11 `DeleteUserUseCase` / data-model.md §17)の運用強化: ① **削除前のデータエクスポート機能**(`entry_corpus` / `decline_corpus` / 過去 `entries` を CSV 一括ダウンロード)、② **グレースピリオド 7 日**(削除予約 → 復元可能ソフトデリート → 7 日後に物理削除。グレースピリオドとは「猶予期間」の意で、ユーザーが「やっぱり削除を取り消したい」と思った場合に復元できる安全装置。Google アカウント 20 日 / Microsoft アカウント 30 日 等の業界慣行に倣う)、③ **再登録時の整合性**(グレースピリオド中に同じ `line_user_id` で再登録要求が来た場合、UNIQUE 制約で新規作成をブロック → 「以前のアカウントを復活しますか?」ダイアログを LINE で表示し能動選択させる)、④ **R2 アーカイブのライフサイクル設定**(`audit_logs` の保管期限を法令変更時に短縮可能にする)、⑤ R2 上の `audit_logs` アーカイブ NDJSON 内の `user_id` を月次バッチで匿名化する Cron。MVP は即時削除 + R2 アーカイブ永続のシンプル設計 |
| **P2-10** | コーパス取り込み拡張 | `entry_corpus` / `decline_corpus`(data-model.md §13 / §14)の取り込み運用拡張: ① **取り込み期間のユーザー設定**(全期間 / 直近 12ヶ月 / 直近 24ヶ月 / 直近 36ヶ月 等から選択可)、② **24 ヶ月超のコーパスを R2 にアーカイブ**、Few-shot 採用時に必要なものだけ R2 から取り出す、③ ベテラン MC が「過去の名作 PR を学ばせたい」要望に対応。MVP は直近 24 ヶ月固定で取り込み、永続保管(application-design.md §9 TBD「コーパス取り込み期間」が確定値) |
| **P2-11** | データモデル詳細化 | MVP の最小スキーマで省略した派生情報の追跡を追加: ① **`case_messages` 中間表**(1 案件 N メールを明示関連付け、決定通知メール等のスレッド以外の関連メッセージを記録)、② **`pattern_revisions` 履歴テーブル**(`office_patterns.pattern_data` 更新後の成功率変化をリビジョンごとに追跡、Few-shot 品質測定用)。MVP は `cases.source_message_id` 単一 + `office_patterns` 永続累積 `success_count` で簡略化 |
| **P2-12** | 税務関連長期保管 | 個人事業主 MC の **税務上の記録保管要件**(所得税法 5〜7 年)に対応: ① 確定 `entries` / `cases` / `compensation_amount` を **税務年度単位で R2 にアーカイブ**、② ユーザーがいつでも CSV でダウンロード可能、③ Phase 2 P2-08(請求 CSV)の長期保管基盤として機能。MVP は構造化データ 24 ヶ月で D1 から物理削除(税務利用しない前提) |
...
| F-10 | — | NFR-4 SECURITY-13(ベース URL 確定 / DNS 設定 / TLS) |
| F-11 | — | NFR-3(拡張性ポート、D-15〜D-18.5 全体)、NFR-7(IdP / Calendar / Mail プロバイダ差し替え時の改修コスト局所化) |
| F-12 | — | NFR-3(C4 図による全体像可視化)、NFR-8(オンボーディング向け資料の前提) |
| F-13 | — | NFR-6(信頼性 / 回帰検出)、NFR-1(性能テスト)、NFR-4 SECURITY-04(入力検証テスト)|
| F-14 | — | NFR-3(可用性 SLO 監視)、NFR-4 SECURITY-14(セキュリティアラート)、NFR-7(コスト超過アラート) |
| U1-01 | FR-1 | — |
...
|------|---------|----------------|--------------|----------------|
| Foundation | 14 | 2 (F-03, F-07) | 12 (F-01/02/04/05/06/08/09/10/11/12/13/14) | 0 |
| Use Case 1 (F1) | 6 | 3 | 0 | 3 |
| Use Case 2 (F2) | 9 | 4 | 1 (U2-00) | 4 (含 U2-EC-04) |
...
> - Foundation Epic に **F-13(テスト戦略・E2E)/ F-14(監視・アラート設計)** を追加
> - U7-02 に過去辞退メールからの Few-shot 学習を追加
