...
- **AC-1 (下書き生成 — 過去辞退メールから Few-shot 学習)**:
  - **Given** 重複する仮案件のリストと、過去にユーザーが送信した辞退メール(`pr_corpus` と並列の `decline_corpus` テーブル、Sent フォルダから自動収集)
  - **When** 辞退下書き生成が走る
...
| **P2-09** | アカウントデータ管理拡張 | アカウント削除請求(A-11 `DeleteUserUseCase` / data-model.md §17)の運用強化: ① **削除前のデータエクスポート機能**(`pr_corpus` / `decline_corpus` / 過去 `entries` を CSV 一括ダウンロード)、② **グレースピリオド 7 日**(削除予約 → 復元可能ソフトデリート → 7 日後に物理削除)、③ **R2 アーカイブのライフサイクル設定**(`audit_logs` の保管期限を法令変更時に短縮可能にする)、④ R2 上の `audit_logs` アーカイブ NDJSON 内の `user_id` を月次バッチで匿名化する Cron。MVP は即時削除 + R2 アーカイブ永続のシンプル設計 |
| **P2-10** | コーパス取り込み拡張 | `pr_corpus` / `decline_corpus`(data-model.md §13 / §14)の取り込み運用拡張: ① **取り込み期間のユーザー設定**(全期間 / 直近 12ヶ月 / 直近 24ヶ月 / 直近 36ヶ月 等から選択可)、② **24 ヶ月超のコーパスを R2 にアーカイブ**、Few-shot 採用時に必要なものだけ R2 から取り出す、③ ベテラン MC が「過去の名作 PR を学ばせたい」要望に対応。MVP は直近 24 ヶ月固定で取り込み、永続保管(application-design.md §9 TBD「コーパス取り込み期間」が確定値) |
