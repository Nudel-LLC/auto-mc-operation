...
## 構成方針(Plan セクション 8 準拠 + P2 ペルソナ反映)
...
- **AC-2 (3 環境の役割と権限を明記)**:
  ...
  - **Then** 以下の役割と権限が `docs/environments.md` に明記され、Cloudflare API トークンが各環境に分離される:
    ...
    |------|------|-------------|---------------|----------------------------|
    | **dev** | ローカル開発・単体テスト | 開発者本人(`wrangler dev`) | 開発者本人のみ・モックデータ | モック / ステージング Bot |
    | **staging** | β テスト・統合テスト | 開発者(CI 経由) | 開発者 + βテスター(自データのみ) | 専用 staging LINE Bot / staging Google アプリ |
    | **production** | 本番運用 | 開発者(CI 経由・main ブランチ承認後) | エンドユーザー(自データのみ) + 運用者(管理 API のみ) | production LINE Bot / production Google アプリ |
...
- **AC-1 (用語の言い換え辞書 + AI への遵守強制)**:
  ...
  - **Note (用語解説)**: ここでの「メッセージ定数化」とは、ユーザー向け文言をコード本文に直書きせず別ファイルに集約する習慣のこと(将来の多言語対応・一括レビュー・lint 適用が容易になる)。「i18n」(internationalization の略) という業界用語は、このプロジェクト内では使用しない
- **AC-2 (失敗時の次アクション必須化)**:
  - **Given** エラー通知メッセージ
  - **When** メッセージを生成する
...
- **AC-1 (シークレット分類)**:
  ...
  - **When** 設計レビュー
  - **Then** `docs/secrets-management.md` に以下が明記される:
    | 種類 | 保管場所 | アクセス経路 | ローテーション頻度 |
    |------|---------|-------------|-------------------|
    | Anthropic API キー(2 本) | Wrangler secrets(`PRIMARY` / `SECONDARY`) | env binding | 90 日 / 漏洩疑い時即時 |
    | LINE Channel access token / secret | Wrangler secrets(環境別) | env binding | 90 日 |
    | Google OAuth Client Secret | Wrangler secrets | env binding | Google Cloud で管理 |
    | OAuth リフレッシュトークン暗号化鍵(`TOKEN_ENC_KEY`) | Wrangler secrets | env binding | 180 日 |
    | LINE Webhook 署名検証用 secret | Wrangler secrets | env binding | LINE 側ローテに追従 |
- **AC-2 (リフレッシュトークン暗号化)**:
  - **Given** D1 に保管する OAuth リフレッシュトークン
  - **When** 永続化する
  - **Then** **AES-256-GCM** で暗号化され、暗号鍵は `TOKEN_ENC_KEY`(Wrangler secrets)から取得する
  - **Then** ノンスはレコードごとにランダム生成し、`{nonce}:{ciphertext}:{tag}` 形式で保存
- **AC-3 (鍵ローテーション戦略)**:
  - **Given** すべてのシークレットに 2 本構成または **キー識別子付き(key-id)** の保管設計
  - **When** ローテーションを実施する
  - **Then** 無停止で旧鍵から新鍵へ移行できる(復号は両鍵を試行、暗号化は新鍵のみ使用、既存レコードはバックグラウンド再暗号化)
- **AC-4 (シークレットの非ログ化)**:
  - **Given** ロガー / エラーレポート
...
- **AC-1 (第 1 段: ルールベース分類)**:
  ...
  - **Then** **まず Anthropic API を呼ばずに** ルールベース分類を試みる:
    - 既知 allowlist(登録済み事務所ドメイン × 「募集」「お知らせ」キーワード)→ `recruitment`
    - 既知 allowlist × 「決定」「採用」「ご担当」キーワード → `decision`
    - blocklist(noreply / promotion 系ドメイン)→ `other`(U1-03 と統合)
...
|------|---------|----------------|--------------|----------------|
| Foundation | 9 | 2 (F-03, F-07) | 7 (F-01/02/04/05/06/08/09) | 0 |
...
| Phase 2 | 7 (概略) | — | — | — |
| **合計(MVP+Foundation)** | **56** | **36** | **7** | **13** |

> Foundation Epic に F-08(P2 配慮)/ F-09(暗号鍵管理)を追加し、計 56 Story で MVP を網羅。粒度をさらに細かく要望される場合は次回ループで分割可能です。
