...
1. **事前確認**: PRテンプレート（`.github/pull_request_template.md`）への準拠チェック
2. **コードレビュー**: `git diff main...HEAD` の差分を5視点でレビュー
3. **総合評価**: 判定とサマリーを出力

> **注意**: 事前確認でCriticalが見つかった場合、コードレビューを中断してPR説明の修正を求めます。
...
コードレビューの前に、PRの説明文が `.github/pull_request_template.md` に沿って記入されているかを確認します。
...
| 変更内容 | 🔴 Critical |
| 関連Issue | 🟡 Warning |
| 動作確認チェックボックス | 🔴 Critical |
| レビュー観点 | 🟡 Warning |

Criticalが1件でもある場合、Claudeはコードレビューを中断しPR説明の修正を促します。
